...
function rateFor(rank) {
  if (["SDP","SP","SE"].includes(rank)) return 500000;
  if (["MOD","MOD+","MOD++"].includes(rank)) return 1000000;
  if (["ADM","ADM+","ADM++"].includes(rank)) return 1500000;
  return 0;
}
const STAFF_RANKS = {
  HIGH: ["ADM","ADM+","ADM++"],
...
app.get("/api/bootstrap", requireAuth, (req,res) => {
  ...
  res.json({
    ...
    rates:{HIGH:1500000,MID:1000000,LOW:500000},
    ranks:STAFF_RANKS,
    timeZone:TIME_ZONE
  });
});
